...
async function summarizeDate(groupId, dateStr) {
  ...
  const promptText = `นี่คือบทสนทนาในกลุ่มไลน์วันที่ ${dateStr} (ข้อความที่ขึ้นต้นด้วย [ส่งรูปภาพ: xxx] คือตำแหน่งที่มีการส่งรูปภาพในเวลานั้น แต่ไม่ได้แนบตัวรูปมาให้วิเคราะห์):\n\n${conversation}\n\nแยกประเด็น/เรื่องสำคัญที่คุยกันในวันนี้ออกเป็นรายการ แต่ละประเด็นให้สรุปตามหลัก 5W1H เป็นภาษาไทย เพื่อนำไปใส่ในตาราง โดยให้ **ละเอียดครบถ้วน**:\n- เขียนเป็นประโยคที่สมบูรณ์ อ่านแล้วเข้าใจบริบทได้เต็มที่ ไม่ต้องตัดทอนเป็นวลีสั้นๆ\n- ใส่รายละเอียดที่เกี่ยวข้องทั้งหมดที่ปรากฏในบทสนทนา เช่น ตัวเลข วันที่ ชื่อเฉพาะ เงื่อนไข หรือบริบทประกอบ อย่าตัดทิ้งเพียงเพราะยาว\n- ไม่จำกัดความยาวต่อฟิลด์ แต่ละฟิลด์ยาวเท่าที่จำเป็นเพื่อสื่อความให้ครบ\n- ห้ามพูดข้อมูลซ้ำกันระหว่างฟิลด์ (เช่นถ้าบอกสาเหตุใน why แล้ว ไม่ต้องพูดซ้ำใน what)\n- ถ้าเรื่องเดียวกันถูกพูดถึงหลายครั้งในวัน ให้รวมเป็นประเด็นเดียว โดยรวมรายละเอียดจากทุกครั้งที่พูดถึงเข้าด้วยกัน อย่าแยกซ้ำ\n- ถ้าบทสนทนาไม่ได้ระบุข้อมูลของฟิลด์ไหนไว้ชัดเจน ให้ใส่ "ไม่ระบุ" (ไม่ต้องเดา)\nฟิลด์ที่ต้องมี:\n- what: อะไร (ประเด็นคืออะไร)\n- who: ใคร (บุคคล/ทีม/แผนก)\n- when: เมื่อไหร่\n- where: ที่ไหน (สถานที่/เครื่องจักร/ไลน์ผลิต)\n- why: ทำไม (สาเหตุ)\n- how: อย่างไร (วิธีแก้/Action Item)\nถ้าทั้งวันไม่มีสาระสำคัญเลย ให้ตอบ topics เป็น array ว่าง []\n\nตอบกลับเป็น JSON เท่านั้น ห้ามมีข้อความอื่นนอกเหนือจาก JSON ในรูปแบบ:\n{"topics": [{"what": "...", "who": "...", "when": "...", "where": "...", "why": "...", "how": "..."}]}`;
